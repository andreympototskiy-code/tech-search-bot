...
def debug_world_devices():
    ...
    if product_elements:
        ...
        # Ищем цены
        price_texts = first_product.find_all(
            text=lambda text: text and ("р." in str(text) or "руб" in str(text))
        )
...
def debug_pitergsm():
    ...
    query = "iPhone 15 PRO 256gb"
    search_url = (
        f"https://pitergsm.ru/?digiSearch=true&term={query}&params=%7Csort%3DDEFAULT"
    )
    ...
    print(
        f"📄 Заголовок страницы: {soup.find('title').get_text() if soup.find('title') else 'Не найден'}"
    )
